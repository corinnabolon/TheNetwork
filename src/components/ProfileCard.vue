  ...
  <section class="row">
    <div class="col-11 light-brown-bg rounded shadow m-3">
      <div class="d-flex flex-column">
        <div class="profileCard-coverImg mt-3" :title="`${profileProp.name}'s cover image`">
        ...
        <div class="fs-1 d-flex justify-content-end">
          ...
          <a v-if="profileProp.linkedin" :href="profileProp.linkedin" :title="`${profileProp.name}'s LinkedIn`"><i
              class="mdi mdi-linkedin mx-3"></i></a>
          <a v-if="profileProp.resume" :href="profileProp.resume" :title="`${profileProp.name}'s Resume`"><i
              class="mdi mdi-text-box-outline mx-3"></i></a>
        </div>
        <div class="mt-5 ms-5">
          <p>{{ profileProp.class }}
        ...
        <router-link v-if="profileProp.id == account.id" :to="{ name: 'Account' }">
          <button class="btn btn-theme mb-3" title="Edit Your Account Information">Edit Account Information</button>
        </router-link>
      </div>
    </div>
  </section>
